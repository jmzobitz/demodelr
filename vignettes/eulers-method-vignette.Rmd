...
In order to make Euler's method work, you will need three things:
...
- A shorthand name for the variables $\vec{y}$ in your differential equation (i.e. $y_{1}=P$ , $y_{2}=V$) that we have in our differential equation.
...
# library(devtools)
# install_github("jmzobitz/MAT369Code")
library(MAT369Code)
...
Next you need shorthand names of your parameters.  We use the R command `c` (as in "collection") to refer to your variable names.  These must be the same variables you use in your differential equations.

```{r, fig.show='hold'}
variableNames <- c('V')  # Be sure to use single quotes for the names of the variables.
```
...
#initialCondition <- c(1, 3)  # Be sure you have enough conditions as you do variables.

initialCondition <- c(1)  # Be sure you have enough conditions as you do variables.

# To do multiple initial conditions, here is some sample code:
# initialCondition <- rbind(c(1, 3),  # Separate out initial conditions with a separate row.
#                        c(5,6),
#                        c(10,9) )
```
...
#     return(c(dV, dP))  # <-- If you have more equations you will need to list the dVariables
#   })
# }
...
dynamics <- function(t, state, parameters){
  with(as.list(c(state, parameters)), {  # Do not edit this line
    dV = -r*V
    return(c(dV))  # <-- If you have more equations you will need to list the dVariables
  })
}
...
Now we have everything we need!  Basically the next step is to run the command `euler`, which will print to the screen a plot of your equation.  If you have multiple initial conditions, you will get a stacked plot for each run.
