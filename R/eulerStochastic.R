...
eulerStochastic <- function(deltaT=1,timeSteps=1,initialCondition,FUN=deterministicDynamics,FUN_2=stochasticDynamics,parameters=parameters,nSimulations=1,sigma=1) {
  ...
  for(j in 1:nSimulations) {
    ...
    for (i in 2:timeSteps) {
      oldP = newP
      newP <-  unlist(deterministicDynamics(time[i],oldP,parameters))*deltaT+ unlist(stochasticDynamics(time[i],oldP,parameters))*sigma*sqrt(deltaT)*rnorm(nVariables)+oldP   # Your differential equation goes here.
